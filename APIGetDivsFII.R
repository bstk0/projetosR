...
# -------------------------------------
# -------------------------------------------------------------------------------
# OBTER TOTAL DE DIVIDENDOS INFORMANDO MES E ANO
# IMPORTANTE : CARREGAR OS DADOS DE SIGLA, MOVIMENTOS E DIVIDENDOS SEPARADAMENTE ....
# -------------------------------------------------------------------------------
# -------------------------------------
#---
...
#---
# SAIDA FINAL - Depois de carregar, pode somente ir alterando aqui os parametros
# pode ser executado informando outros meses
...
for (i in 1:dim(df_siglas)[1]) {
 ...
 t2 <- getQtdeTotalAcoes(df_sum, p_id = df_siglas$id[i],p_ano = 2022, p_mes = s_mes) #QTDE
 t3 <- getValorDiv(df_div, p_id = df_siglas$id[i], p_anomes = s_anomes)          #DIV
...
# Exibe o TOTAL ...
print(sum(as.numeric(df_list$TOTAL)))
# ---------------------------------------------

View(df_list)

# VALIDA CARGA - COMPARANDO TOTAL COM CSV
validaCarga <- filter(df_div, ano_mes == s_anomes )
print(sum(validaCarga$valor))
